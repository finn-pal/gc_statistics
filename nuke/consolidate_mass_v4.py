...
def create_time_dict(mass_dict, num_step=1000):
...
def get_mass_by_formation(it, gc_mass_dict, time_dict):
    it_id = iteration_name(it)
    ...
    #######################################################################################
    for gc_id in gc_mass_dict[it_id]:
        type_flag = gc_mass_dict[it_id][gc_id]["type_flag"]

        time_lst = gc_mass_dict[it_id][gc_id]["time"]
        form_time_lst = gc_mass_dict[it_id][gc_id]["form_time"]

        log_mass_lst = gc_mass_dict[it_id][gc_id]["log_mass"]

        mass_loss_lst = gc_mass_dict[it_id][gc_id]["mass_loss"]
        mass_loss_det_lst = gc_mass_dict[it_id][gc_id]["mass_loss_detectable"]
        ...
        # accretion time
        if type_flag > 1:
            acc_time_lst = gc_mass_dict[it_id][gc_id]["acc_time"]
...
if __name__ == "__main__":
    ...
    # make them first individually (with mp) and then add them together

    with mp.Manager() as manager:
        # shared_dict_0 = manager.dict()  # Shared dictionary across processes
        args = [(it, evolve_mass_loss, sim, sim_dir) for it in it_lst]

        with mp.Pool(processes=cores, maxtasksperchild=1) as pool:
            results_gc_mass = pool.starmap(get_mass_data, args, chunksize=1)

    # gc_mass_dict = {}
    # for result in results_gc_mass:
    #     it_id = result[0]
    #     it_mass_dict = result[1]

    #     gc_mass_dict[it_id] = {}
    #     gc_mass_dict[it_id] = it_mass_dict

    gc_mass_dict = {it_id: it_mass_dict for it_id, it_mass_dict in results_gc_mass}
    ...
    time_dict = create_time_dict(gc_mass_dict)
    ...
    with mp.Manager() as manager:
        # shared_dict_1 = manager.dict()  # Shared dictionary across processes

        args = [(it, gc_mass_dict, time_dict) for it in it_lst]

        with mp.Pool(processes=cores, maxtasksperchild=1) as pool:
            results_gc_type_mass = pool.starmap(get_mass_by_formation, args, chunksize=1)

        # type_dict = dict(shared_dict_1)

    # type_dict = {}
    # for result in results_gc_type_mass:
    #     it_id = result[0]
    #     it_type_dict = result[1]

    #     type_dict[it_id] = {}
    #     type_dict[it_id] = it_type_dict

    type_dict = {it_id: it_type_dict for it_id, it_type_dict in results_gc_type_mass}
