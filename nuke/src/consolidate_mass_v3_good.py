...
def get_mass_by_formation(it, gc_mass_dict, time_dict, type_dict):
    it_id = iteration_name(it)
...
if __name__ == "__main__":
    ...
    with mp.Manager() as manager:
        ...
        args = [(it, evolve_mass_loss, sim, sim_dir, shared_dict_0) for it in it_lst]

        with mp.Pool(processes=cores, maxtasksperchild=1) as pool:
            pool.starmap(get_mass_data, args, chunksize=1)

        gc_mass_dict = dict(shared_dict_0)
    ...
    with mp.Manager() as manager:
        ...
        args = [(it, gc_mass_dict, time_dict, shared_dict_1) for it in it_lst]

        with mp.Pool(processes=cores, maxtasksperchild=1) as pool:
            pool.starmap(get_mass_by_formation, args, chunksize=1)
