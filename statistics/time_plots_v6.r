...
get_time_dep <- function(
  ...
  group_dict_template <- list(
    ...
    avg_vy_grp = numeric(),
    avg_vz_grp = numeric(),
    avg_vphi_grp = numeric(),
    avg_vr_grp = numeric(),
    avg_r_cyl_grp = numeric(), # new
    avg_phi_cyl_grp = numeric(), # new
    avg_r_grp = numeric(), # new
    avg_r_apo_grp = numeric(), # new
    avg_r_per_grp = numeric(), # new
    avg_ecc_grp = numeric(), # new
    sig_vx_grp = numeric(),
    sig_vy_grp = numeric(),
    sig_vz_grp = numeric(),
    sig_vphi_grp = numeric(),
    sig_vr_grp = numeric(),
    num_gc = integer()
  )
  ...
  for (group_id in group_ids) {
    ...
    for (snap_id in snap_id_lst) {
      ...
      vphi <- snap_dat[["vphi_cyl"]][]
      vr <- snap_dat[["vr_cyl"]][]

      r_cyl <- snap_dat[["r_cyl"]][]
      phi_cyl <- snap_dat[["phi_cyl"]][]

      r <- snap_dat[["r"]][]
      r_apo <- snap_dat[["r_apo"]][]
      r_per <- snap_dat[["r_per"]][]
      ...
      vx_grp <- vx[(snap_grp_lst == group_id) & (bnd_flag == 1)]
      vy_grp <- vy[(snap_grp_lst == group_id) & (bnd_flag == 1)]
      vz_grp <- vz[(snap_grp_lst == group_id) & (bnd_flag == 1)]

      vphi_grp <- vphi[(snap_grp_lst == group_id) & (bnd_flag == 1)]
      vr_grp <- vr[(snap_grp_lst == group_id) & (bnd_flag == 1)]

      r_cyl <- r_cyl[(snap_grp_lst == group_id) & (bnd_flag == 1)]
      phi_cyl <- phi_cyl[(snap_grp_lst == group_id) & (bnd_flag == 1)]

      r <- r[(snap_grp_lst == group_id) & (bnd_flag == 1)]
      r_apo <- r_apo[(snap_grp_lst == group_id) & (bnd_flag == 1)]
      r_per <- r_per[(snap_grp_lst == group_id) & (bnd_flag == 1)]

      ecc <- ecc[(snap_grp_lst == group_id) & (bnd_flag == 1)]
      ...
      group_dict$avg_vx_grp <-
        c(group_dict$avg_vx_grp, mean(vx_grp, na.rm = TRUE))
      group_dict$avg_vy_grp <-
        c(group_dict$avg_vy_grp, mean(vy_grp, na.rm = TRUE))
      group_dict$avg_vz_grp <-
        c(group_dict$avg_vz_grp, mean(vz_grp, na.rm = TRUE))
      group_dict$avg_vphi_grp <-
        c(group_dict$avg_vphi_grp, mean(vphi_grp, na.rm = TRUE))
      group_dict$avg_vr_grp <-
        c(group_dict$avg_vr_grp, mean(vr_grp, na.rm = TRUE))
      group_dict$avg_r_cyl_grp <-
        c(group_dict$avg_r_cyl_grp, mean(r_cyl, na.rm = TRUE))
      group_dict$avg_phi_cyl_grp <-
        c(group_dict$avg_phi_cyl_grp, mean(phi_cyl, na.rm = TRUE))
      group_dict$avg_r_grp <-
        c(group_dict$avg_r_grp, mean(r, na.rm = TRUE))
      group_dict$avg_r_apo_grp <-
        c(group_dict$avg_r_apo_grp, mean(r_apo, na.rm = TRUE))
      group_dict$avg_r_per_grp <-
        c(group_dict$avg_r_per_grp, mean(r_per, na.rm = TRUE))
      group_dict$avg_ecc_grp <-
        c(group_dict$avg_ecc_grp, mean(ecc, na.rm = TRUE))
      group_dict$sig_vx_grp <-
        c(group_dict$sig_vx_grp, sd(vx_grp, na.rm = TRUE))
      group_dict$sig_vy_grp <-
        c(group_dict$sig_vy_grp, sd(vy_grp, na.rm = TRUE))
      group_dict$sig_vz_grp <-
        c(group_dict$sig_vz_grp, sd(vz_grp, na.rm = TRUE))
      group_dict$sig_vphi_grp <-
        c(group_dict$sig_vphi_grp, sd(vphi_grp, na.rm = TRUE))
      group_dict$sig_vr_grp <-
        c(group_dict$sig_vr_grp, sd(vr_grp, na.rm = TRUE))
      group_dict$num_gc <-
        c(group_dict$num_gc, length(lz_norm_grp))
    }
...
it_dict_list <- mclapply(it_lst, function(it) {
  get_time_dep(it, cont_level, h_dict, sim_dir, data_dir)
}, mc.cores = num_cores)
