...
plot_contours_ind <- function(group_id, group_col, it_lst, snap_lst, save_dir, sim, sim_dir, h_dict, cont = 75,
    ...
    if (length(frame_paths) > 0) {
        ...
        image_write(gif, path = file.path(save_dir, paste0("group_", group_id, "_contour.gif")))
    } else {
...
plot_contours_grp <- function(group_ids, group_cols, it_lst, snap_lst, save_dir, sim, sim_dir, h_dict, cont = 75,
                              var_1 = "lz_norm", var_2 = "et_norm",
                              var_1_axis = expression(epsilon), var_2_axis = "e") {
    # Open the HDF5 file
    proc_path <- file.path(sim_dir, sim, paste0(sim, "_processed.hdf5"))
    ...
    for (i in seq_along(snap_lst)) {
        ...
        first_group <- TRUE # before the group loop
        for (k in seq_along(group_ids)) {
            ...
            if (length(valid_kdes) == 0) {
                next # Skip this group if no valid KDEs
            } else if (length(valid_kdes) == 1) {
                kde_est_avg <- valid_kdes[[1]]
            } else {
        ...
        offset <- 0.02 * diff(par("usr")[3:4]) # adjust as needed
        x_range <- diff(par("usr")[1:2]) # width of plot
        ...
        for (g in seq_along(group_labels)) {
            ...
            if (g == 2) {
                ...
                x_pos <- x_pos + 0.03 * x_range # shift each label horizontally
            }
            text(
                x = x_pos,
                y = par("usr")[4] - offset,
