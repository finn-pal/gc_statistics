...
# Flatten the estimate matrix
z_vals0 <- as.vector(kde0$estimate)
z_vals1 <- as.vector(kde1$estimate)

# Sort and compute cumulative density
sorted_z0 <- sort(z_vals0, decreasing = TRUE)
cum_probs0 <- cumsum(sorted_z0) / sum(sorted_z0)

sorted_z1 <- sort(z_vals1, decreasing = TRUE)
cum_probs1 <- cumsum(sorted_z1) / sum(sorted_z1)

# Find the cutoff for 75% mass
level_750 <- sorted_z0[which(cum_probs0 >= 0.75)[1]]
level_751 <- sorted_z1[which(cum_probs1 >= 0.75)[1]]

# png("kde_75pct_contour.png", width = 800, height = 800)

# Plot the KDE
# image(kde_result$eval.points[[1]], kde_result$eval.points[[2]], kde_result$estimate,
#     xlab = "X", ylab = "Y", main = "2D KDE with 75% Contour", col = topo.colors(100)
# )
...
# print(kde1$cont)
cont <- 75
hts <- contourLevels(kde0, prob = (100 - cont) / 100, approx = TRUE)
...
plot(kde0, cont = 75, col = "blue", drawlabels = FALSE, lwd = 1, add = TRUE)
