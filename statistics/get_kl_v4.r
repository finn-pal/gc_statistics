...
for (i in seq_along(snap_lst)) {
    ...
    for (j in seq_along(it_lst)) {
        ...
        var_1_grp_vals <- var_1_vals[(snap_grp == group_id) & (bnd_flag == 1)]
        var_2_grp_vals <- var_2_vals[(snap_grp == group_id) & (bnd_flag == 1)]

        kde_i <- get_kde_from_vals(var_1_grp_vals, var_2_grp_vals, bandwidth)

        kde_list[[j]] <- kde_i$estimate
        x_all_list[[j]] <- kde_i$x

        if (j == 1) {
            eval_points <- kde_i$eval.points
        }
    }
